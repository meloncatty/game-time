...
let isDead = null;
// let score = document.getElementById('user-score').innerHTML;
...
function endGame() {
  if(newGame.lives <= 0) {
    newGame.lives = 4;
    var initials = prompt('Enter your initials!', 'XXX');
    var score = newGame.score;
    return true;
  }
}

function scoreToStorage(initials, score) {
  var userScores = new Scores(score, initials);
  var stringifyScores = JSON.stringify(userScores);
  localStorage.setItem(userScores.id, stringifyScores);
}
